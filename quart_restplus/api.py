...
from quart.exceptions import HTTPException, HTTPStatusException, MethodNotAllowed, NotFound
...
# List headers that should never be handled by Quart-RESTPlus
...
class Api:
    ...
    You need to initialize it with a Quart Application: ::

    >>> app = Quart(__name__)
    >>> api = Api(app)

    Alternatively, you can use :meth:`init_app` to set the Quart application
    ...
    :param quart.Quart|quart.Blueprint app: the Quart application object or a Blueprint
    ...
    def __init__(self, app=None, version='1.0', title=None, description=None,
                 ...
                 authorizations=None, security=None, doc: Union[str, bool] = '/', default_id=default_id,
                 default='default', default_label='Default namespace', validate=None,
                 tags=None, prefix='', ordered=False,
                 default_mediatype: Union[str, None] = 'application/json', decorators=None,
    ...
    def init_app(self, app, **kwargs):
        """
        Allow to lazy register the API on a Quart application::

        >>> app = Quart(__name__)
        >>> api = Api()
        >>> api.init_app(app)

        :param quart.Quart app: the Quart application object
        :param str title: The API title (used in Swagger documentation)
        :param str description: The API description (used in Swagger documentation)
        :param str terms_url: The API terms page URL (used in Swagger documentation)
        :param str contact: A contact email for the API (used in Swagger documentation)
        ...
        if isinstance(app, quart.Blueprint):
    ...
    def _init_app(self, app):
        """
        Perform initialization actions with the given :class:`quart.Quart` object.
    ...
    def _register_view(self, app, resource, namespace, *urls, **kwargs):
        ...
        # NOTE: 'view_functions' is cleaned up from Blueprint class in Quart 1.0
        ...
        view_func = resource.as_view(endpoint, self, *resource_class_args, **resource_class_kwargs)
        if getattr(view_func, 'methods', None) is None:
            delattr(view_func, 'methods')
        resource_func = self.output(view_func)
    ...
    def output(self, resource):
        """
        Wraps a resource (as a quart view function),
        for cases where the resource does not directly return a response object

        :param resource: The resource as a quart view function
        """
        ...
        async def wrapper(*args, **kwargs):
            ...
            return await self.make_response(data, code, headers=headers)

        return wrapper

    async def make_response(self, data, *args, **kwargs):
        """
        ...
        if mediatype is None:
            raise NotAcceptable
        elif mediatype in self.representations:
            resp = await self.representations[mediatype](data, *args, **kwargs)
            resp.headers['Content-Type'] = mediatype
            return resp
        elif mediatype == 'text/plain':
            resp = await original_quart_make_response(str(data), *args)
    ...
    def __schema__(self):
        ...
        if not self._schema:
            try:
                ...
                log.error(msg, exc_info=e)  # This will provide a full traceback
                return {'error': msg}
        return self._schema
    ...
    def _should_use_qr_error_handler(self):
        """
        Determine if error should be handled with FR or default Quart

        The goal is to return Quart error handlers for non-FR-related routes,
        ...
        try:
            ...
            adapter.method = list(e.allowed_methods)[0]
    ...
    def _has_qr_route(self):
        """Encapsulating the rules for whether the request was to a Quart endpoint"""
        # 404's, 405's, which might not have a url_rule
        if self._should_use_qr_error_handler():
            return True
    ...
    async def error_router(self, original_handler, e):
        """
        This function decides whether the error occured in a quart-restplus
        endpoint or not. If it happened in a quart-restplus endpoint, our
        handler will be dispatched. If it happened in an unrelated view, the
        app's original error handler will be dispatched.
        In the event that the error occurred in a quart-restplus endpoint but
    ...
    async def handle_error(self, e):
        ...
        """
        await got_request_exception.send(current_app._get_current_object(), exception=e)
        ...
        resp = await self.make_response(data, code, headers, fallback_mediatype=fallback_mediatype)
    ...
    def _help_on_404(self, message=None):
        rules = dict([(RE_RULES.sub('', rule.rule), rule.rule)
                      for rule in current_app.url_map.rules])
    ...
    @staticmethod
    def _blueprint_setup_add_url_rule_patch(state: quart.blueprints.BlueprintSetupState,
        ...
        if callable(path):
            path = path(state.url_prefix)
        elif state.url_prefix:
            path = state.url_prefix + path

        if endpoint is None:
            endpoint = _endpoint_from_view_func(view_func)

        blueprint: quart.Blueprint = state.blueprint
        if subdomain is None:
            subdomain = blueprint.subdomain

        state.app.add_url_rule(path,
                               '%s.%s' % (blueprint.name, endpoint),
                               view_func,
                               methods,
                               defaults,
                               host,
    ...
    def _deferred_blueprint_init(self, setup_state):
        ...
        perform initialization with setup_state.app :class:`quart.Quart` object.
        When a :class:`quart_restplus.Api` object is initialized with a blueprint,
        this method is recorded on the blueprint to be run when the blueprint is later
        registered to a :class:`quart.Quart` object.  This method also monkeypatches
        ...
        :type setup_state: quart.blueprints.BlueprintSetupState
        ...
        if not setup_state.first_registration:
            raise ValueError('quart-restplus blueprints can only be registered once.')
    ...
    def representation(self, mediatype):
        ...
        * A dictionary of headers
        ...
        and return a Quart response object.
    ...
    def unauthorized(self, response):
        ...
        if self.serve_challenge_on_401:
            realm = current_app.config.get("HTTP_BASIC_AUTH_REALM", "quart-restplus")
