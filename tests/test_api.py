...
async def test_root_endpoint_lazy(app):
...
def test_doc_with_inheritance(app):
    api = restplus.Api(app, prefix='/api', version='1.0')
...
def test_specs_endpoint_not_added(app):
    api = restplus.Api()
...
async def test_specs_endpoint_not_found_if_not_added(app, client):
    api = restplus.Api()
    api.init_app(app, add_specs=False)
...
async def test_default_endpoint(app):
...
async def test_default_endpoint_lazy(app):
    api = restplus.Api()
...
async def test_default_endpoint_with_blueprint_with_subdomain(app):
    app.url_map.host_matching = True
    app.config['SERVER_NAME'] = 'localhost'
    blueprint = Blueprint('api', __name__, subdomain='api', url_prefix='/api')
    api = restplus.Api(blueprint)
    ...
    async with app.test_request_context():
        assert url_for('api.test_resource', _external=True) == 'http://api.localhost/api/test/'


async def test_default_endpoint_for_namespace(app):
    api = restplus.Api(app)
...
async def test_default_endpoint_lazy_for_namespace(app):
    api = restplus.Api()
    ns = api.namespace('ns', 'Test namespace')
...
async def test_multiple_default_endpoint(app):
    api = restplus.Api(app)
...
async def test_multiple_default_endpoint_lazy(app):
    api = restplus.Api()
...
async def test_multiple_default_endpoint_for_namespace(app):
    api = restplus.Api(app)
    ns = api.namespace('ns', 'Test namespace')
...
async def test_multiple_default_endpoint_lazy_for_namespace(app):
    api = restplus.Api()
    ns = api.namespace('ns', 'Test namespace')
...
async def test_multiple_default_endpoint_for_namespace_with_blueprint(app):
    blueprint = Blueprint('api', __name__, url_prefix='/api')
    api = restplus.Api(blueprint)
    ns = api.namespace('ns', 'Test namespace')
...
async def test_ns_path_prefixes(app):
    api = restplus.Api()
...
async def test_multiple_ns_with_authorizations(app):
    api = restplus.Api()
    a1 = {
        'apikey': {
            'type': 'apiKey',
...
def test_non_ordered_namespace(app):
    api = restplus.Api(app)
    ns = api.namespace('ns', 'Test namespace')
...
def test_ordered_namespace(app):
...
def test_decorators(app, mocker):
